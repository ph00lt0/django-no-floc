...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-no-floc runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-no-floc.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ no_floc")
...
def lint(c):
    ...
    """
    c.run("flake8 django-no-floc tests")
...
def release(c, bumpsize=''):
    ...
    import django_no_floc
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_no_floc.__version__))
    c.run("git push --tags")
